...
# 注册定时任务（追加，不覆盖Dockerfile预设crontab）
chmod +x /data/stock/jobs/cron.9h/run_morning_report
chmod +x /data/stock/jobs/cron.14h/run_evening_report
(crontab -l 2>/dev/null; echo "28 9 * * 1-5 bash /data/stock/jobs/cron.9h/run_morning_report"; echo "50 14 * * 1-5 bash /data/stock/jobs/cron.14h/run_evening_report") | crontab -
